...
- запись исходящего сообщения только после успешной доставки;
...
6. Модель должна вызвать ровно один инструмент:
   ...
   - `send_to_both`;
   - `no_action`.
7. Ответ доставляется в Telegram.
8. Исходящее сообщение записывается в историю только после успешной доставки.
